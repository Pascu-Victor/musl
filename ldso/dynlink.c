...
#include "../src/malloc/mallocng/glue.h"
#include "fork_impl.h"
#include "libc.h"
...
#define fpaddr(p, v) fdbarrier((&(struct funcdesc){laddr(p, v), (p)->got}))
#else
...
static void decode_vec(size_t *v, size_t *a, size_t cnt) {
    ...
    for (; v[0]; v += 2)
        if (v[0] - 1 < cnt - 1) {
            if (v[0] < 8 * sizeof(long)) a[0] |= 1UL << v[0];
            a[v[0]] = v[1];
        }
}
...
static Sym *sysv_lookup(const char *s, uint32_t h, struct dso *dso) {
    ...
    for (i = hashtab[2 + h % hashtab[0]]; i; i = hashtab[2 + hashtab[0] + i]) {
        if ((!dso->versym || dso->versym[i] >= 0) && (!strcmp(s, strings + syms[i].st_name))) return syms + i;
    }
    return 0;
}
...
static Sym *gnu_lookup(uint32_t h1, uint32_t *hashtab, struct dso *dso, const char *s) {
    ...
    for (h1 |= 1;; i++) {
        uint32_t h2 = *hashval++;
        if ((h1 == (h2 | 1)) && (!dso->versym || dso->versym[i] >= 0) && !strcmp(s, dso->strings + dso->syms[i].st_name))
            return dso->syms + i;
        if (h2 & 1) break;
    }
...
#endif
static inline struct symdef
find_sym2(struct dso *dso, const char *s, int need_def, int use_deps) {
...
static struct symdef find_sym(struct dso *dso, const char *s, int need_def) { return find_sym2(dso, s, need_def, 0); }
...
static struct symdef get_lfs64(const char *name) {
    ...
    if (!strcmp(name, "readdir64_r")) return find_sym(&ldso, "readdir_r", 1);
    size_t l = strnlen(name, 18);
    if (l < 2 || name[l - 2] != '6' || name[l - 1] != '4' || name[l]) goto nomatch;
    for (p = lfs64_list; *p; p++) {
        if (!strncmp(name, p, l - 2) && !p[l - 2]) return find_sym(&ldso, p, 1);
        while (*p) p++;
    }
nomatch:
...
static void do_relocs(struct dso *dso, size_t *rel, size_t rel_size, size_t stride) {
    ...
    for (; rel_size; rel += stride, rel_size -= stride * sizeof(size_t)) {
        ...
        if (sym_index) {
            ...
            def = (sym->st_info >> 4) == STB_LOCAL ? (struct symdef){.dso = dso, .sym = sym} : find_sym(ctx, name, type == REL_PLT);
        ...
        if ((type == REL_TPOFF || type == REL_TPOFF_NEG) && def.dso->tls_id > static_tls_cnt) {
            error(
        ...
        switch (type) {
            ...
            case REL_TLSDESC:
                ...
                if (def.dso->tls_id > static_tls_cnt) {
                    ...
                    if (!new) {
                        error("Error relocating %s: cannot allocate TLSDESC for %s", dso->name, sym ? name : "(local)");
                        longjmp(*rtld_fail, 1);
                    }
        ...
        if ((relr[0] & 1) == 0) {
            ...
            *reloc_addr++ += (size_t)base;
            ...
            for (size_t bitmap = relr[0]; (bitmap >>= 1); i++)
                if (bitmap & 1) reloc_addr[i] += (size_t)base;
...
static void reclaim_gaps(struct dso *dso) {
    ...
    for (; phcnt--; ph = (void *)((char *)ph + dso->phentsize)) {
        ...
        reclaim(dso, ph->p_vaddr + ph->p_memsz, ph->p_vaddr + ph->p_memsz + PAGE_SIZE - 1 & -PAGE_SIZE);
    }
}
...
static void *map_library(int fd, struct dso *dso) {
    ...
    for (i = eh->e_phnum; i; i--, ph = (void *)((char *)ph + eh->e_phentsize)) {
        if (ph->p_type == PT_DYNAMIC) {
            ...
            if (!runtime && ph->p_memsz > __default_stacksize) {
                __default_stacksize = ph->p_memsz < DEFAULT_STACK_MAX ? ph->p_memsz : DEFAULT_STACK_MAX;
            }
        }
        if (ph->p_type != PT_LOAD) continue;
        nsegs++;
        if (ph->p_vaddr < addr_min) {
            ...
            prot =
                (((ph->p_flags & PF_R) ? PROT_READ : 0) | ((ph->p_flags & PF_W) ? PROT_WRITE : 0) | ((ph->p_flags & PF_X) ? PROT_EXEC : 0));
        }
    ...
    if (DL_FDPIC && !(eh->e_flags & FDPIC_CONSTDISP_FLAG)) {
        ...
        for (ph = ph0, i = 0; i < nsegs; ph = (void *)((char *)ph + eh->e_phentsize)) {
            ...
            prot =
                (((ph->p_flags & PF_R) ? PROT_READ : 0) | ((ph->p_flags & PF_W) ? PROT_WRITE : 0) | ((ph->p_flags & PF_X) ? PROT_EXEC : 0));
            map = mmap(0, ph->p_memsz + (ph->p_vaddr & PAGE_SIZE - 1), prot, MAP_PRIVATE, fd, ph->p_offset & -PAGE_SIZE);
            if (map == MAP_FAILED) {
                unmap_library(dso);
                goto error;
            }
            ...
            if (prot & PROT_WRITE) {
                ...
                if (pgend > pgbrk &&
                    mmap_fixed(map + pgbrk, pgend - pgbrk, prot, MAP_PRIVATE | MAP_FIXED | MAP_ANONYMOUS, -1, off_start) == MAP_FAILED)
                    goto error;
    ...
    map = DL_NOMMU_SUPPORT ? mmap((void *)addr_min, map_len, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)
    ...
    for (ph = ph0, i = eh->e_phnum; i; i--, ph = (void *)((char *)ph + eh->e_phentsize)) {
        ...
        if (!dso->phdr && eh->e_phoff >= ph->p_offset && eh->e_phoff + phsize <= ph->p_offset + ph->p_filesz) {
        ...
        if ((ph->p_vaddr & -PAGE_SIZE) != addr_min || DL_NOMMU_SUPPORT)
            if (mmap_fixed(base + this_min, this_max - this_min, prot, MAP_PRIVATE | MAP_FIXED, fd, off_start) == MAP_FAILED) goto error;
        if (ph->p_memsz > ph->p_filesz && (ph->p_flags & PF_W)) {
            size_t brk = (size_t)base + ph->p_vaddr + ph->p_filesz;
            size_t pgbrk = brk + PAGE_SIZE - 1 & -PAGE_SIZE;
            memset((void *)brk, 0, pgbrk - brk & PAGE_SIZE - 1);
            if (pgbrk - (size_t)base < this_max && mmap_fixed((void *)pgbrk, (size_t)base + this_max - pgbrk, prot,
    ...
    for (i = 0; ((size_t *)(base + dyn))[i]; i += 2)
        if (((size_t *)(base + dyn))[i] == DT_TEXTREL) {
            if (mprotect(map, map_len, PROT_READ | PROT_WRITE | PROT_EXEC) && errno != ENOSYS) goto error;
            break;
        }
done_mapping:
    dso->base = base;
...
static int fixup_rpath(struct dso *p, char *buf, size_t buf_size) {
    ...
    if (libc.secure && *origin != '/') return 0;
...
static struct dso *load_library(const char *name, struct dso *needed_by) {
    ...
    if (name[0] == 'l' && name[1] == 'i' && name[2] == 'b') {
        ...
        for (rp = reserved; *rp; rp = next) {
            next = strchr(rp, '.') + 1;
            if (strncmp(name + 3, rp, next - rp) == 0) break;
        }
        if (*rp) {
            if (ldd_mode) {
    ...
    if (strchr(name, '/')) {
        ...
        if (fd == -1) {
            if (!sys_path) {
                ...
                snprintf(etc_ldso_path, sizeof etc_ldso_path, "%.*s/etc/ld-musl-" LDSO_ARCH ".path", (int)prefix_len, prefix);
                fd = open(etc_ldso_path, O_RDONLY | O_CLOEXEC);
                if (fd >= 0) {
                    size_t n = 0;
                    if (!fstat(fd, &st)) n = st.st_size;
                    if ((sys_path = malloc(n + 1))) sys_path[n] = 0;
    ...
    decode_dyn(&temp_dso);
    if (find_sym(&temp_dso, "__libc_start_main", 1).sym && find_sym(&temp_dso, "stdin", 1).sym) {
...
#ifdef TLS_ABOVE_TP
        p->tls.offset = tls_offset + ((p->tls.align - 1) & (-tls_offset + (uintptr_t)p->tls.image));
...
#endif
        p->new_dtv = (void *)(-sizeof(size_t) & (uintptr_t)(p->name + strlen(p->name) + sizeof(size_t)));
...
static void load_direct_deps(struct dso *p) {
    ...
    if (p == head)
        for (struct dso *q = p->next; q; q = q->next) cnt++;
    ...
    p->deps = (p == head && cnt < 2) ? builtin_deps : calloc(cnt + 1, sizeof *p->deps);
    if (!p->deps) {
    ...
    cnt = 0;
    if (p == head)
        for (struct dso *q = p->next; q; q = q->next) p->deps[cnt++] = q;
    for (i = 0; p->dynv[i]; i += 2) {
        if (p->dynv[i] != DT_NEEDED) continue;
        struct dso *dep = load_library(p->strings + p->dynv[i + 1], p);
        if (!dep) {
            error("Error loading shared library %s: %m (needed by %s)", p->strings + p->dynv[i + 1], p->name);
            if (runtime) longjmp(*rtld_fail, 1);
            continue;
        }
        p->deps[cnt++] = dep;
...
static void load_deps(struct dso *p) {
    if (p->deps) return;
    for (; p; p = p->next) load_direct_deps(p);
}
...
static void extend_bfs_deps(struct dso *p) {
    ...
    int no_realloc = (__malloc_replaced && !p->runtime_loaded) || p->deps == builtin_deps;

    if (p->bfs_built) return;
    ndeps_all = p->ndeps_direct;

    /* Mark existing (direct) deps so they won't be duplicated. */
    for (i = 0; p->deps[i]; i++) p->deps[i]->mark = 1;
    ...
    for (i = 0; p->deps[i]; i++) {
        ...
        tmp = no_realloc ? malloc(sizeof(*tmp) * (ndeps_all + cnt + 1)) : realloc(p->deps, sizeof(*tmp) * (ndeps_all + cnt + 1));
        if (!tmp) {
...
static void reloc_all(struct dso *p) {
    ...
    for (; p; p = p->next) {
        ...
        if (!DL_FDPIC) do_relr_relocs(p, laddr(p, dyn[DT_RELR]), dyn[DT_RELRSZ]);

        if (head != &ldso && p->relro_start != p->relro_end) {
            long ret = __syscall(SYS_mprotect, laddr(p, p->relro_start), p->relro_end - p->relro_start, PROT_READ);
            if (ret != 0 && ret != -ENOSYS) {
...
void __libc_exit_fini() {
    ...
    for (p = fini_head; p; p = p->fini_next) {
        while (p->ctor_visitor && p->ctor_visitor != self) pthread_cond_wait(&ctor_cond, &init_fini_lock);
...
static struct dso **queue_ctors(struct dso *dso) {
    ...
    if (dso->bfs_built) {
        for (cnt = 0; dso->deps[cnt]; cnt++) dso->deps[cnt]->mark = 0;
        cnt++; /* self, not included in deps */
    } else {
        for (cnt = 0, p = head; p; cnt++, p = p->next) p->mark = 0;
    }
...
static void do_init_fini(struct dso **queue) {
    ...
    pthread_mutex_lock(&init_fini_lock);
    for (i = 0; (p = queue[i]); i++) {
        while ((p->ctor_visitor && p->ctor_visitor != self) || shutting_down) pthread_cond_wait(&ctor_cond, &init_fini_lock);
        if (p->ctor_visitor || p->constructed) continue;
        p->ctor_visitor = self;
...
void __libc_start_init(void) {
    ...
    if (!__malloc_replaced && main_ctor_queue != builtin_ctor_queue) free(main_ctor_queue);
    main_ctor_queue = 0;
}
...
static void update_tls_size() {
    ...
    libc.tls_size = ALIGN((1 + tls_cnt) * sizeof(void *) + tls_offset + sizeof(struct pthread) + tls_align * 2, tls_align);
}
...
static void install_new_tls(void) {
    ...
    for (i = 0, td = self; !i || td != self; i++, td = td->next) {
        memcpy(newdtv + i, td->dtv, (old_cnt + 1) * sizeof(uintptr_t));
        newdtv[i][0] = tls_cnt;
    }
    ...
    for (p = head;; p = p->next) {
        ...
        for (j = 0; j < i; j++) {
            ...
            newdtv[j][p->tls_id] = (uintptr_t) new + DTP_OFFSET;
...
void __dls3(size_t *sp, size_t *auxv) {
    ...
    libc.secure = ((aux[0] & 0x7800) != 0x7800 || aux[AT_UID] != aux[AT_EUID] || aux[AT_GID] != aux[AT_EGID] || aux[AT_SECURE]);
    ...
    if (aux[AT_PHDR] != (size_t)ldso.phdr) {
        ...
        if (!argv[0]) {
            dprintf(2,
                    ...
                    __libc_version, ldname, ldd_mode ? "" : " [args]");
            _exit(1);
        }
    ...
    load_deps(&app);
    for (struct dso *p = head; p; p = p->next) add_syms(p);
    ...
    if (search_vec(auxv, &vdso_base, AT_SYSINFO_EHDR) && vdso_base) {
        ...
        for (i = ehdr->e_phnum; i; i--, phdr = (void *)((char *)phdr + ehdr->e_phentsize)) {
            ...
            if (phdr->p_type == PT_LOAD) vdso.base = (void *)(vdso_base - phdr->p_vaddr + phdr->p_offset);
        }
        vdso.name = "";
    ...
    if (find_sym(head, "malloc", 1).dso != &ldso) __malloc_replaced = 1;
    if (find_sym(head, "aligned_alloc", 1).dso != &ldso) __aligned_alloc_replaced = 1;
...
static void prepare_lazy(struct dso *p) {
    ...
    if (dyn[DT_BIND_NOW] || (dyn[DT_FLAGS] & DF_BIND_NOW) || (flags1 & DF_1_NOW)) return;
...
void *dlopen(const char *file, int mode) {
    ...
    if (!p) {
        error(noload ? "Library %s is not already loaded" : "Error loading shared library %s: %m", file);
        goto end;
    }
    ...
    if (!p->relocated && (mode & RTLD_LAZY)) {
        prepare_lazy(p);
        for (i = 0; p->deps[i]; i++)
            if (!p->deps[i]->relocated) prepare_lazy(p->deps[i]);
    }
    if (!p->relocated || (mode & RTLD_GLOBAL)) {
        /* Make new symbols global, at least temporarily, so we can do
         * relocations. If not RTLD_GLOBAL, this is reverted below. */
        add_syms(p);
        for (i = 0; p->deps[i]; i++) add_syms(p->deps[i]);
    }
    if (!p->relocated) {
        reloc_all(p);
    }
    ...
    if (!(mode & RTLD_GLOBAL)) revert_syms(orig_syms_tail);
    ...
    for (p = head; p; p = p->next) {
        if (DL_FDPIC && p->loadmap) {
            ...
            for (; phcnt--; ph = (void *)((char *)ph + entsz)) {
                if (ph->p_type != PT_LOAD) continue;
                if (a - base - ph->p_vaddr < ph->p_memsz) return p;
            }
            if (a - (size_t)p->map < p->map_len) return 0;
        }
    }
    return 0;
}
...
static void *do_dlsym(struct dso *p, const char *s, void *ra) {
    ...
    if ((def.sym->st_info & 0xf) == STT_TLS) return __tls_get_addr((tls_mod_off_t[]){def.dso->tls_id, def.sym->st_value - DTP_OFFSET});
    if (DL_FDPIC && (def.sym->st_info & 0xf) == STT_FUNC) return def.dso->funcdescs + (def.sym - def.dso->syms);
...
int dladdr(const void *addr_arg, Dl_info *info) {
    ...
    if (DL_FDPIC) {
        size_t idx = (addr - (size_t)p->funcdescs) / sizeof(*p->funcdescs);
    ...
    if (!best)
        for (; nsym; nsym--, sym++) {
            if (sym->st_value && (1 << (sym->st_info & 0xf) & OK_TYPES) && (1 << (sym->st_info >> 4) & OK_BINDS)) {
    ...
    if (DL_FDPIC && (bestsym->st_info & 0xf) == STT_FUNC) best = (size_t)(p->funcdescs + (bestsym - p->syms));
...
int dl_iterate_phdr(int (*callback)(struct dl_phdr_info *info, size_t size, void *data), void *data) {
    ...
    for (current = head; current;) {
        ...
        info.dlpi_tls_data = !current->tls_id ? 0 : __tls_get_addr((tls_mod_off_t[]){current->tls_id, 0});
