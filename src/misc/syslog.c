...
static int is_lost_conn(int e) { return e == ECONNREFUSED || e == ECONNRESET || e == ENOTCONN || e == EPIPE; }
...
static void _vsyslog(int priority, const char *message, va_list ap) {
    ...
    l = snprintf(buf, sizeof buf, "<%d>%s %n%s%s%.0d%s: ", priority, timebuf, &hlen, log_ident, "["[!pid], pid, "]"[!pid]);
    errno = errno_save;
    ...
    if (l2 >= 0) {
        ...
        if (send(log_fd, buf, l, 0) < 0 &&
            (!is_lost_conn(errno) || connect(log_fd, (void *)&log_addr, sizeof log_addr) < 0 || send(log_fd, buf, l, 0) < 0) &&
            (log_opt & LOG_CONS)) {
