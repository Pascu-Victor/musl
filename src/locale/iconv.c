...
static iconv_t combine_to_from(size_t t, size_t f) { return (void *)(f << 16 | t << 1 | 1); }
...
iconv_t iconv_open(const char *to, const char *from) {
    ...
    if ((t = find_charmap(to)) == -1 || (f = find_charmap(from)) == -1 || (charmaps[t] >= 0330)) {
        errno = EINVAL;
        return (iconv_t)-1;
    }
        ...
        switch (type) {
            ...
            case UTF_16:
                ...
                if (!scd->state) {
                    if (*inb < 2) goto starved;
                    c = get_16((void *)*in, 0);
                    scd->state = type == UCS2 ? c == 0xfffe ? UCS2LE : UCS2BE : c == 0xfffe ? UTF_16LE : UTF_16BE;
            ...
            case BIG5:
                ...
                if (c - 0xa1 >= 0xfa - 0xa1) {
                    ...
                    if (c < 0xa1)
                        ...
                        c -= 0x87 + (0xfa - 0xa1);
                    c = (hkscs[4867 + (c * 157 + d) / 16] >> (c * 157 + d) % 16) % 2 << 17 | hkscs[c * 157 + d];
