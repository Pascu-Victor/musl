...
char *bindtextdomain(const char *domainname, const char *dirname) {
    ...
    for (q = bindings; q; q = q->next) {
        if (!strcmp(q->domainname, domainname) && q != p) a_store(&q->active, 0);
    }
...
static char *dummy_gettextdomain() { return "messages"; }
...
char *dcngettext(const char *domainname, const char *msgid1, const char *msgid2, unsigned long int n, int category) {
    ...
    for (q = bindings; q; q = q->next)
        if (!strcmp(q->domainname, domainname) && q->active) break;
    if (!q) goto notrans;
    ...
    for (p = cats; p; p = p->next)
        if (p->binding == q && p->lm == lm && p->cat == category) break;
    ...
    if (!p) {
        ...
        for (;;) {
            snprintf(name, sizeof name, "%s/%.*s%.*s/%s/%s.mo\0", dirname, (int)loclen, locname, (int)alt_modlen, modname, catname,
                     domainname);
