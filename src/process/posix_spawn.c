...
static int child(void *args_vp) {
    ...
    if (attr->__flags & POSIX_SPAWN_SETSID)
        if ((ret = __syscall(SYS_setsid)) < 0) goto fail;
    ...
    if (attr->__flags & POSIX_SPAWN_RESETIDS)
        if ((ret = __syscall(SYS_setgid, __syscall(SYS_getgid))) || (ret = __syscall(SYS_setuid, __syscall(SYS_getuid)))) goto fail;
            ...
            switch (op->cmd) {
                ...
                case FDOP_DUP2:
                    ...
                    if (fd != op->fd) {
                        ...
                        ret = __syscall(SYS_fcntl, fd, F_SETFD, ret & ~FD_CLOEXEC);
                        if (ret < 0) goto fail;
                    }
                    break;
                case FDOP_OPEN:
