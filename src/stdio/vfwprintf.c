...
static const char sizeprefix['y' - 'a'] = {['a' - 'a'] = 'L', ['e' - 'a'] = 'L', ['f' - 'a'] = 'L', ['g' - 'a'] = 'L', ['d' - 'a'] = 'j',
...
static int wprintf_core(FILE *f, const wchar_t *fmt, va_list *ap, union arg *nl_arg, int *nl_type) {
    ...
    for (;;) {
        ...
        snprintf(charfmt, sizeof charfmt, "%%%s%s%s%s%s*.*%c%c", "#"[!(fl & ALT_FORM)], "+"[!(fl & MARK_POS)], "-"[!(fl & LEFT_ADJ)],
                 " "[!(fl & PAD_POS)], "0"[!(fl & ZERO_PAD)], sizeprefix[(t | 32) - 'a'], t);
